...
import { organizationDashboardAtom } from "@/_recoil/states";
import { combineAndRemoveDuplicates, moveValueToEnd, removeItemOnce, ucword } from "@/helpers";
import Radio from "@/components/ui/radio";


export default function Defects() {
    const [selectedValue, setSelectedValue] = useState("all");
    const projectDashboard: any = useRecoilValue(organizationDashboardAtom);
    ...
    return (
        <div className=" rounded-2xl border border-gray-200 bg-white px-5 pt-5 dark:border-gray-800 dark:bg-white/[0.03] sm:px-6 sm:pt-6">
            <div className="flex items-start justify-between flex-col">
                ...
                <div className="relative inline-block mt-2 overflow-auto max-h-[400px] w-full gap-6 flex flex-col z-1">
                    {
                                    ...
                                    {(selectedValue == "all" || selectedValue == "properties") && (
                                        <div className="flex w-full items-center gap-3">
                                            <div className="relative block h-2 w-full rounded bg-gray-200 dark:bg-gray-600 basis-[90%]">
                                                <div
                                                    className={`absolute left-0 top-0 flex h-full items-center justify-center rounded bg-blue-500 text-xs font-medium text-white`}
                                                    style={{
                                                        width: getPercent(
                                                            projectDashboard &&
