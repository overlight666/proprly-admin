...
import { useChecklist, useCountriesAction, useDefect, useOrganization, useProject } from "@/_recoil/actions";
import { allProjectsAtom, commonAreaChecklistElementListAtom, organizationsAtom, regionsAtom } from "@/_recoil/states";
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
        ...
        <div className="rounded-lg border border-gray-300 shadow-theme-xs">
          <div className="flex justify-between p-2 items-center border-gray-300 shadow-theme-xs">
            ...
            <div
              className={`cursor-pointer ${isType == "default" || (isType !== "default" && selectedId)
                ? "text-blue-600"
                : "text-gray-600"
                }`}
              onClick={() => {
          ...
          <div className="max-h-[500px] overflow-auto">
            {checklistElements?.map((zone: any, index) => {
              return (
                <div
                  key={index}
                  className={`flex justify-between p-2 items-center ${defaultSelectedZone === index &&
                ...
                >
                  ...
                  >
                    <span
                      className={`${!zone.isActive && "line-through text-red-900"
                        }`}
                    >
                      {zone.name}
                    </span>
        ...
        <div className="rounded-lg border border-gray-300 shadow-theme-xs">
          <div className="flex justify-between p-2 items-center border-gray-300 shadow-theme-xs">
            ...
            <div
              className={`cursor-pointer ${isType ? "text-blue-600" : "text-gray-600"
                }`}
              onClick={() => {
                if (isType) {
                  setModalType(1);
            ...
            {selectedElement?.elements?.map((element: any, index) => {
              return (
                <div key={index} className="flex w-full flex-col">
                  <div
                    className={`flex justify-between p-2 items-center ${defaultSelectedElement === index &&
                      "bg-gray-200 dark:bg-gray-700"
                      }`}
                  >
                    <div
                    ...
                    >
                      <span
                        className={`${!element.isActive && "line-through text-red-900"
                          }`}
                      >
                        {element.name}
                      </span>
                      ...
                      <ul className="list-disc">
                        {element?.subElements?.map(
                          (subs: any, subIndex: any) => {
                            return (
                              <li
                                className={`py-1 px-1 hover:bg-gray-200 hover:dark:bg-gray-700 cursor-pointer ${defaultSelectedSubElement === subIndex &&
