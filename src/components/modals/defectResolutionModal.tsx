...
export default function DefectResolutionModal({
    ...
    const setSidebarIndex = useSetRecoilState(sidebarIndexAtom);
    ...
    const mutateIcon = (name: any): any => {
        const splitName = name.split("-");
        let newName = "mdi"
        splitName?.map((names: any) => {
            newName = newName + ucword(names)
        })
        return newName
    }

    const setDefectMessage = (msg: any) => {
        toast.info(msg)
    }
    ...
    return (
        <>
            <Modal
                isOpen={isOpen}
                onClose={closeModal}
                className="max-w-[80%] p-6 lg:p-10 max-h-[90%] relative overflow-hidden z-99999"
            >
                <div className="flex flex-col px-2">
                                                ...
                                                <li className="mb-4 ms-4" key={index}>
                                                    ...
                                                    <time className="mb-1 text-sm font-normal leading-none text-gray-400 dark:text-gray-500">
                                                        {config &&
                                                            config?.roles &&
                                                            config.roles.find(
                                                                (c: any) => c.id == activity?.loggedUserRoleId
                                                            )?.roleName}
                                                    </time>
                                                    <div className="flex">
                                                        <div
                    ...
                    <hr className="mt-5" />
                    {
                        defect?.approvalOptions?.length > 0 && <div className="flex items-center gap-3 mt-6 modal-footer sm:justify-start">
                            {
                                defect?.approvalOptions?.find((approval) => approval?.key == "close") && <Button variant="outline"
                                    onClick={() => {
                            ...
                            {
                                defect?.approvalOptions?.find((approval) => approval?.key == "reopen") && <Button variant="default"
                                    onClick={() => {
                                        defectAction
                            ...
                            {
                                defect?.approvalOptions?.find((approval) => approval?.key == "reject") && <Button variant="destructive"
                                    onClick={() => {
                                        defectAction.pushDefectFeedback(defect?.id, {
                                            feedback: "reject",
                                        }).then(() => {
                            ...
                            {
                                defect?.approvalOptions?.find((approval) => approval?.key == "accept") && <Button variant="default"
                                    onClick={() => {
                                        defectAction
                                            .pushDefectFeedback(defect?.id, {
                                                feedback: "accept",
