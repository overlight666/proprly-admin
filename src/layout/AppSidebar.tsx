      ...
      {items.map((nav, index) => (
        <li key={nav.name}>
          {nav.subItems ? (
            ...
            >
              <span
                className={`menu-item-icon-size  min-w-[37px] min-h-[37px] max-w-[37px] max-h-[37px] ${openSubmenu?.type === menuType && openSubmenu?.index === index
            ...
            nav.path && (
              <Link
                to={nav.path}
                className={`menu-item group${!isExpanded && !isHovered && !isMobileOpen && "justify-center"
                  } ${isActive(nav.path)
                    ? "menu-item-active items-center"
                    : (!isActive(nav.path) &&
                      !selectedProject &&
              ...
              >
                <span
                  className={`menu-item-icon-size min-w-[37px] min-h-[37px] max-w-[37px] max-h-[37px] ${isActive(nav.path)
                    ? "menu-item-icon-active"
                    : (!isActive(nav.path) &&
                      !selectedProject &&
                      selectedOrganization?.name == nav.name) ||
