...
import { globalConfigAtom, projectSalesAgentAtom, selectedProjectAtom } from "@/_recoil/states";
...
export const SalesAgent = () => {
        ...
        if (selectedProject) {
            const attachedUsers = selectedProject?.user?.filter(
                (u: userInterface) =>
                    u.project_role &&
                    u.project_role.find(
                        (role: projectRole) => role.roleKey === "project_sales_agent"
                    )
            );
    ...
    return (
        <div className="space-y-6 px-5 py-5">
            ...
            />
            <div>
                <Label htmlFor="inputTwo">Sales Agent list</Label>
