...
const MultiSelect: React.FC<MultiSelectProps> = ({
    ...
    useEffect(() => {
        const removeSelected = options?.filter(
            (op: any) => !defaultSelected.includes(op.value)
        );

        setFullOption(removeSelected);
    }, []);

    const handleSelect = (optionValue: string) => {
        const newSelectedOptions: any = selectedOptions.includes(optionValue)
            ? selectedOptions.filter((value) => value !== optionValue)
            : [...selectedOptions, optionValue];

        setSelectedOptions(newSelectedOptions);
        onChange?.(newSelectedOptions);
        const newOptions = fullOption.filter(
            (value: any) => value.value !== optionValue
        );
        setFullOption(newOptions);
    };

    const removeOption = (value: string) => {
        const newSelectedOptions = selectedOptions.filter((opt) => opt !== value);
        const getOption = options.find((o: any) => o.value === value);
        setFullOption((oldArray: any) => [...oldArray, getOption]);
        setSelectedOptions(newSelectedOptions);
        onChange?.(newSelectedOptions);
    };
            ...
            <div className="relative z-20 inline-block w-full">
                <div className="relative flex flex-col items-center">
                    <div onClick={toggleDropdown} className="w-full">
                        <div className="mb-2 flex min-h-11 rounded-lg border border-gray-300 py-1.5 pl-3 pr-3 shadow-theme-xs outline-none transition focus:border-brand-300 focus:shadow-focus-ring dark:border-gray-700 dark:bg-gray-900 dark:focus:border-brand-300">
                            ...
                            <div className="flex items-center py-1 pl-1 pr-1 w-7">
                                ...
                                >
                                    <svg
                                        className={`stroke-current ${isComponentVisible ? "rotate-180" : ""
                                            }`}
                                        width="20"
                                        height="20"
                                        viewBox="0 0 20 20"
                    ...
                    <div ref={ref}>
                        {isComponentVisible && (
                            <div
                                className="absolute left-0 z-99999 w-full overflow-auto bg-white rounded-lg shadow top-full  dark:bg-gray-900 max-h-[400px]"
